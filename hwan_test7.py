...
def play_next_video1(output: str):
    if st.session_state.current_video_index < len(st.session_state.video_sources):
        word, source = st.session_state.video_sources[st.session_state.current_video_index]
        if source == "Not found":
            st.write(f"No video found for the word: {word}")
            st.session_state.current_video_index += 1
            play_next_video1(output=output)
            return
        ...
        output_text_container.markdown(
            f"<div class='output-text'>수어 응답: {output}</div>",
            unsafe_allow_html=True
        )

def play_next_video2(input: str):
    if st.session_state.current_video_index < len(st.session_state.video_sources):
        word, source = st.session_state.video_sources[st.session_state.current_video_index]
        if source == "Not found":
            st.write(f"No video found for the word: {word}")
            st.session_state.current_video_index += 1
            play_next_video2(input=input)
            return
        ...
        left_text_container.markdown(
            f"<div class='output-text'>{input}</div>",
            unsafe_allow_html=True
        )
...
def display_chat_messages():
    # CSS 스타일 정의 (이미 정의되어 있으므로 중복되지 않도록 주의)
    st.markdown("""
        <style>
        .white-container {
    ...
    # 하얀 컨테이너 안에 모든 메시지 포함
    st.markdown('<div class="white-container chat-container">', unsafe_allow_html=True)
    ...
    for msg in st.session_state.chat_messages:
        ...
        if len(current_group) == 4:  # 한 사용자 메시지와 세 시스템 메시지
            conversation_groups.append(current_group)
            current_group = []
    
    if current_group:  # 남은 메시지 처리
        conversation_groups.append(current_group)
    ...
    for group in conversation_groups:
        for idx, msg in enumerate(group):
            ...
            # 버튼 처리
            if idx == 0:
                user_input = group[0].get("message", "")
                user_input1 = group[1].get("message", "")
                
                # 버튼을 위한 열 생성
                col1, col2, col3 = st.columns([9, 0.3, 0.7])
                
                # 버튼을 오른쪽 열에 배치
                with col3:
                    if st.button(
                        "입력 보기", 
                        key=f"play_{timestamp}_{user_input}", 
                        help="Click to see sign language videos for this conversation"
                    ):
                        video_output_right.empty()
                        output_text_container.empty()
                        words = user_input
                        paths = _TexttoSign.find_videos1(words)
                        st.session_state.video_sources = paths
                        st.session_state.current_video_index = 0
                        play_next_video2(input=user_input1)
            
            if idx == 2:
                system_input = group[3].get("message", "")
                system_output = group[2].get("message", "")
                if st.button(
                    "응답 보기", 
                    key=f"play_{timestamp}_{system_input}", 
                    help="Click to see sign language videos for this conversation"
                ):
                    video_output_right.empty()
                    output_text_container.empty()
    ...
    with open(log_file_path, "r", encoding="utf-8") as log_file:
        ...
        if len(existing_data) > 0 and len(st.session_state.chat_messages) == 0:
            for entry in existing_data:
                ...
                st.session_state.chat_messages.append({
                    "isUser": False,
                    "message": f"{entry.get('수어변환', '데이터 없음')}",
                    "timestamp": current_time
                })
    ...
    if not st.session_state.is_recognizing:
        ...
        if st.session_state.recognized_words:
            ...
            new_messages = [
                ...
                {
                    "isUser": False,
                    "message": f"수어 변환: {api.ssc_result}",
                    "timestamp": current_time
                }
            ]
