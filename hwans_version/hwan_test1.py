...
def play_next_video():
    if st.session_state.current_video_index < len(st.session_state.video_sources):
        ...
        # result : str = api.cmc_result
        
        output_text_container.markdown(
            # f"<div class='output-text'>수어 응답: {result}</div>",
...
if start_button:
    if not st.session_state.is_recognizing:
        ...
        if st.session_state.recognized_words:
            ...
            # log.txt 파일에서 이미 저장된 값 읽기 (recognized_text만 중복 방지)
            try:
                with open("log.txt", "r", encoding="utf-8") as log_file:
                    existing_lines = set(line.strip() for line in log_file.readlines())  # 중복 제거를 위해 set 사용
            except FileNotFoundError:
                existing_lines = set()  # 파일이 없으면 빈 set 초기화

            # recognized_text 저장 (중복 방지)
            with open("log.txt", "a", encoding="utf-8") as log_file:
                if recognized_text not in existing_lines:
            ...
            st.session_state.current_video_index = 0
            result1 = api.sfc_result
            result2 = api.cmc_result 
            result3 = api.ssc_result 

            play_next_video()

						# API 결과 저장 (중복 허용)
            with open("log.txt", "a", encoding="utf-8") as log_file:
                log_file.write(result1 + "\n")
                log_file.write(result2 + "\n")
                log_file.write(result3 + "\n")
